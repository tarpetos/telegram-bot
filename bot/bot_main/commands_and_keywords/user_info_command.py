...
from bot.bot_main.main_objects_initialization import dp, store_users_data, token_table, bot, unique_table
...
async def user_data(message: types.Message):
    ...
    if chat_id == user_id:
        ...
        bot_message = await message.reply('This command can be used only in private conversation with bot!')
        await asyncio.sleep(DELETE_TIMEOUT)
        await bot.delete_message(chat_id=chat_id, message_id=bot_message.message_id)
