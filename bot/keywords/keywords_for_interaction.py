...
from bot.commands.commands_utils.for_photo_creation.remake_user_photo import (
    create_new_photo_auto_config,
)
from .keyword_utils.states import ImageSaver
from ..config import (
    dp,
    bot,
    unique_table,
)
from ..commands.commands_utils import currency_cost as cc
from .keyword_utils.check_date_words import check_day
from .keyword_utils.check_document_extension import (
    file_option_selector,
    parse_args,
    get_conversion_path_name,
)
from .keyword_utils.file_size_contoller import send_documents
from bot.commands.commands_utils.get_days_and_date_num import (
    extract_from_user_input_days_and_date,
    extract_from_user_input_days_num,
)
from .keyword_utils.image_to_file_converter import (
    ImageLoader,
    PDFConverter,
    DOCXConverter,
)
from .keyword_utils.morse_options import get_result
from .keyword_utils.remove_start_keyword import (
...
async def image_pdf_converter_filter(message: types.Message, state: FSMContext):
    ...
    await ImageSaver.image.set()
    print(await state.get_data())


@dp.message_handler(content_types=[ContentType.PHOTO, ContentType.DOCUMENT], state=ImageSaver.image)
