...
from .commands_utils.states import TaskScheduler
from .commands_utils import check_for_comma_and_dot, check_for_id_in_table
from ..config import dp, unique_table
from ..enums import Command
from ..keyboards.keyboard_close import close_keyboard
from ..keyboards.task_scheduler import back_to_planner_keyboard
from ..keyboards.task_scheduler.scheduler_keyboard import scheduler_keyboard


@dp.message_handler(state="*", commands=Command.TASK_SCHEDULER)
...
async def option_select(call: types.CallbackQuery):
    select_result = unique_table.select_table(
        "table_" + str(call.from_user.id)
    )
    if not select_result:
        await call.message.edit_text(
            "Table has no records",
            reply_markup=back_to_planner_keyboard.return_keyboard,
        )
    else:
        all_tasks = "".join(
        ...
        await call.message.edit_text(
            all_tasks, reply_markup=back_to_planner_keyboard.return_keyboard
        )
...
async def process_insert(message: types.Message, state: FSMContext):
    try:
        ...
        unique_table.insert_into_table(
            ("table_" + str(message.from_user.id)), message.text
        )
...
async def process_update(message: types.Message, state: FSMContext):
    try:
        ...
        unique_table.update_table(
            f"table_{message.from_user.id}", user_data, int(user_id)
        )
        await message.answer(
        ...
        await message.answer(
            "You have already added this task",
            reply_markup=back_to_planner_keyboard.return_keyboard,
        )
...
async def process_delete(message: types.Message, state: FSMContext):
    try:
        ...
        unique_table.delete_from_table(
            ("table_" + str(message.from_user.id)), message.text
        )
        await message.answer(
        ...
        await message.answer(
            "Task ID was entered incorrectly!",
            reply_markup=back_to_planner_keyboard.return_keyboard,
        )
