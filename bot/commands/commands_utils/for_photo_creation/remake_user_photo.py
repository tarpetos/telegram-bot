...
from .photo_size import write_to_txt
...
def create_new_photo(
    ...
    for word in user_data.split(" "):
        if draw.textlength(text=line + " " + word, font=font) <= image_width - pos_x:
            line += " " + word
        else:
...
def get_image_general_color(image: Image, width, height) -> tuple:
    ...
    for color_counter, color in enumerate(colors, 1):
        color_coefficient = 1 + (color[0] / full_pixels_num)
        sum_red += int(color[1][0] * color_coefficient)
        sum_green += int(color[1][1] * color_coefficient)
        sum_blue += int(color[1][2] * color_coefficient)
