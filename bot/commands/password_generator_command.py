...
from .commands_utils.states import PasswordGeneratorStates
from .commands_utils.generate_password import main_generation

from .commands_utils.password_content_callbacks import (
    default_content_keyboard,
)
from .commands_utils.password_length_callbacks_handler import (
    LengthController,
)
from .commands_utils import check_for_comma_and_dot, check_for_id_in_table
from .commands_utils.check_for_repetitive_characters import (
    check_if_repeatable_characters_is_present,
)
from .commands_utils.check_length_keyboard import keyboard_length_choice
from .commands_utils.check_radio_keyboard import keyboard_content_choice
from .commands_utils.encryption_decryption import encrypt, decrypt
from .commands_utils.process_json import send_json, remove_json, table_is_empty_message
from .commands_utils.message_delete_exception import message_delete_control
from ..config import (
    dp,
    bot,
    unique_table,
...
from ..keyboards.password_generator.back_keyboard import (
    back_to_telegram_generator_keyboard,
)
from ..keyboards.password_generator.download_app_keyboard import download_app_keyboard
from ..keyboards.password_generator.generation_keyboard import (
    third_generator_keyboard,
)
from ..keyboards.password_generator.radio_keyboard import first_generator_keyboard
from ..keyboards.password_generator.set_length_keyboard import (
    second_generator_keyboard,
)
from ..keyboards.password_generator.main_keyboard import password_start_keyboard
from ..keyboards.password_generator.return_to_update_keyboard import (
    password_generator_return_to_update,
)
from ..keyboards.password_generator.telegram_keyboard import (
    password_telegram_keyboard,
)
from ..keyboards.password_generator.update_keyboard import (
    password_generator_update_keyboard,
)

DELETE_TIMEOUT = 60


@dp.message_handler(state="*", commands=Command.PASSWORD_GENERATOR)
...
async def option_show_passwords(call: types.CallbackQuery):
    select_result = unique_table.select_pass_gen_table(
        f"pass_gen_table_{call.from_user.id}"
    )

    if not select_result:
        await call.message.edit_text(
            "Table has no records", reply_markup=back_to_telegram_generator_keyboard
        )
    else:
        all_passwords = "".join(
        ...
        try:
            ...
            await bot.delete_message(
                call.message.chat.id, message_id=sent_message.message_id
            )
...
async def create_password(call: CallbackQuery):
    ...
    await call.message.edit_text(
        text=message_text, parse_mode="HTML", reply_markup=first_generator_keyboard
    )
    await call.answer()
...
async def change_password(call: CallbackQuery):
    ...
    await call.answer(
        'Enter description information in the format: "ID value", "description text"',
        True,
    )
...
async def process_description_update(message: types.Message, state: FSMContext):
    try:
        ...
        if int(user_chosen_id) not in check_for_id_in_table.check_for_password_id(
                message
        ):
            raise ValueError
        ...
        await message.answer(
            "Data has been changed!", reply_markup=password_generator_return_to_update
        )
    except ValueError:
        await message.answer(
            "Entered data was incorrect!",
            reply_markup=password_generator_return_to_update,
        )
...
async def change_password(call: CallbackQuery):
    ...
    await call.answer(
        'Enter password information in the format: "ID value", "password text"', True
    )
    await call.message.delete()
...
async def process_password_update(message: types.Message, state: FSMContext):
    try:
        ...
        repetitive_characters_in_password = check_if_repeatable_characters_is_present(
            user_data
        )
        user_chosen_id = check_for_comma_and_dot.check_user_id(message.text)
        ...
        encrypted_password = encrypt(user_data)
        unique_table.update_password(
            f"pass_gen_table_{message.from_user.id}",
            encrypted_password,
            user_data_length,
            repetitive_characters_in_password,
            int(user_chosen_id),
        )
...
async def delete_password(call: CallbackQuery):
    ...
    await call.answer(
        "Enter password ID and all data about that password will be removed.", True
    )
    await call.message.delete()
...
async def process_password_update(message: types.Message, state: FSMContext):
    try:
        if int(message.text) not in check_for_id_in_table.check_for_password_id(
                message
        ):
            raise ValueError

        unique_table.delete_from_password_table(
            f"pass_gen_table_{message.from_user.id}", message.text
        )
        await message.answer(
        ...
        await message.answer(
            "Password ID was entered incorrectly!",
            reply_markup=back_to_telegram_generator_keyboard,
        )
...
async def password_app(call: CallbackQuery):
    await call.message.edit_text(
        "<b><i>Choose your system: </i></b>",
        parse_mode="HTML",
        reply_markup=download_app_keyboard,
    )
    await call.answer()
...
async def windows_installer(call: CallbackQuery):
    with open(
            "password_generator_app/Password_Generator_Windows_Setup.zip", "rb"
    ) as zip_archive:
...
async def json_password(call: CallbackQuery):
    ...
    if not password_data_list:
        return await table_is_empty_message(call)
    ...
    sent_message = await send_json(call, user_id, password_data_list)
    remove_json(call)
...
async def generate_password(call: types.CallbackQuery):
    ...
    if all(key in data for key in necessary_keys):
        ...
        data.update({"generated_password": generated_password})

        await call.answer(cache_time=2)

        await call.message.edit_text(
            text=f"Your password:\n<code>{generated_password}</code>", parse_mode="HTML"
        )
        await call.message.edit_reply_markup(reply_markup=third_generator_keyboard)
        await storage.set_data(data)
    else:
        await call.answer(
            "You need to press one of the button what your password could "
            "contain and choose password difficulty first!",
            True,
        )
...
async def option_set_description(call: types.CallbackQuery):
    ...
    if all(key in data for key in necessary_keys):
        ...
        await call.answer(
            "You need to press one of the button what your password could "
            "contain, choose password difficulty and generate password first!",
            True,
        )
...
async def process_description(message: types.Message, state: FSMContext):
    ...
    if "generated_password" in data:
        try:
            ...
            user_password = data["generated_password"]
            password_length = keyboard_length_choice(data["password_length"])
            has_repetitive = check_if_repeatable_characters_is_present(user_password)
            ...
            await message.from_user.bot.edit_message_text(
                text=f"<i><b>Password saved successfully!!!</b></i>\n\n"
                     f'Your description:\n<code>{data["description"]}</code>\n'
                     f'Your password:\n<code>{data["generated_password"]}</code>',
...
async def next_to_second(call: CallbackQuery):
    ...
    if "password_length" not in data:
        if all(key in data for key in necessary_keys):
            ...
            await call.message.edit_text(
                text="<i><b>Choose what your password can contain:</b></i>",
                parse_mode="HTML",
            )
...
async def next_to_third(call: CallbackQuery):
    ...
    if "generated_password" in data:
        ...
        if all(key in data for key in necessary_keys):
            ...
            await call.message.edit_text(
                text="<i><b>Choose the password difficulty:</b></i>", parse_mode="HTML"
            )
            await call.message.edit_reply_markup(reply_markup=second_generator_keyboard)
            await call.answer(
...
async def main_generator_menu(call: CallbackQuery):
    ...
    default_content_keyboard()
    LengthController().back_to_default_keyboard()
